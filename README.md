...
SQL stands for Structured Query Language, and it is designed to be able to programmatically perform operations against a database.  SQL databases are relational, as opposed to Mongo which is non-relational.  Having relational data allows records to point to each other using `keys`.
